...
const MovieDetails = () => {
  ...
  const API_DETAIL = `https://api.themoviedb.org/3/movie/${id}?api_key=7f23fe1838341209abe18781249c152f`;
  const VIDEO_DETAIL = `https://api.themoviedb.org/3/movie/${id}/videos?api_key=7f23fe1838341209abe18781249c152f&append_to_response=videos`;
      ...
      <nav className="w-[16%] h-screen hidden xl:flex">
        <ul className="h-full flex flex-col justify-around items-center shadow-xl px-4 border rounded-r-[3rem]">
          ...
          <div className="p-3 m-4 border border-rose-900 rounded-2xl bg-rose-100">
            ...
            <span className="text-[10px] text-rose-900 bg-gray-300 p-1 rounded-lg">
              Start playing
            </span>
          </div>
          <li>
      ...
      <div className="w-full h-full flex items-center justify-center p-1 xl:p-4">
        {findMovie ? (
          <div className="w-full">
            <div className="w-full mb-4 overflow-hidden flex justify-center">
              <ReactPlayer
            ...
            <div className="flex flex-col md:flex-row px-3 md:p-0">
              <div className="basis-[70%] flex flex-col justify-between">
                <div className="flex md:flex-row flex-col items-center text-[#333333]">
                  <div className="flex flex-row items-center text-sm md:text-md font-semibold gap-4 me-4">
                    ...
                    <PiDotOutlineFill />
                    <span data-testid="movie-runtime">{`${Math.floor(
                      runtime / 60
                    )}h  ${runtime % 60}m`}</span>
                  </div>
                  <div className="flex flex-wrap gap-3 mt-2 md:mt-0">
                    {genres.map((genre, index) => (
                      <span
                        key={index}
                        className="text-rose-700 text-sm font-semibold p-1 px-2 border border-rose-200 rounded-full"
                      >
                        {genre.name}
                      </span>
                    ))}
                ...
                <p
                  className="leading-8 my-4 text-sm md:text-md text-[#333333]"
                ...
                <div className="border rounded-md py-2 my-4 text-sm md:text-md">
                  <span className="p-2 w-full bg-rose-700 rounded-md text-white">
                    Top rated movie
                  </span>
              ...
              <div className="basis-[30%] p-6 flex flex-col justify-between text-sm md:text-md">
                ...
                <div className="flex flex-col gap-2">
                  <button className="p-2 w-full bg-rose-700 rounded-md text-white flex items-center justify-center gap-1">
                    <PiTagFill />
                    See Showtimes
                  </button>
                  <button className="p-2 w-full bg-rose-200 border border-rose-700 rounded-md flex items-center justify-center gap-1">
                    <MdMenu />
                    More watch options
                  </button>
                </div>
